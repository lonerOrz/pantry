use gtk4::{
    prelude::*, Application, ApplicationWindow, Box as GtkBox, Label, ListBox, ListBoxRow, Orientation, Overlay, ScrolledWindow,
};
...
impl UiBuilder {
    pub fn build_stdin_ui(
        ...
        app: &Application,
    ) -> (ApplicationWindow, ListBox, Option<Rc<RefCell<preview::PreviewArea>>>, Label) {
        // 从 stdin 读取数据
        ...
        if matches!(display_mode, DisplayMode::Picture) {
            ...
            listbox.connect_selected_rows_changed(move |_listbox| {
                ...
                glib::timeout_add_local(std::time::Duration::from_millis(crate::constants::PREVIEW_UPDATE_THROTTLE_MS), move || {
            ...
            if let Some(paned_widget) = main_widget.downcast_ref::<gtk4::Paned>() {
                ...
                glib::timeout_add_local(std::time::Duration::from_millis(crate::constants::SELECTION_UPDATE_DELAY_MS * 10), move || {
